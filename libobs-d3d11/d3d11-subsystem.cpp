...
void gs_swap_chain::InitTarget(uint32_t cx, uint32_t cy)
{
	...
	target.width = cx;
	target.height = cy;
	// 调用 GetBuffer 方法从交换链中获取第一个缓冲区（索引为 0），并将其作为 ID3D11Texture2D 类型的纹理对象保存到 target.texture 中。
	...
	rtv.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;// 表示这是一个二维纹理的渲染目标视图
	rtv.Texture2D.MipSlice = 0;
	// 调用 CreateRenderTargetView 方法创建渲染目标视图，并将其存储在 target.renderTarget[0] 中。
	...
	if (target.dxgiFormatView == target.dxgiFormatViewLinear) {
		...
		// 更新 rtv.Format 为线性格式 target.dxgiFormatViewLinear，然后创建一个线性渲染目标视图并存储在 target.renderTargetLinear[0] 中。
...
void gs_swap_chain::Resize(uint32_t cx, uint32_t cy, gs_color_format format)
{
	...
	if (swap3) {
		// 如果 swap 可以转换为 IDXGISwapChain3，则根据传入的颜色格式设置合适的颜色空间。调用 SetColorSpace1 方法
...
gs_swap_chain::gs_swap_chain(gs_device *device, const gs_init_data *data)
	: gs_obj(device, gs_type::gs_swap_chain),
	  ...
	  space(GS_CS_SRGB)
{
	DXGI_SWAP_EFFECT effect = DXGI_SWAP_EFFECT_DISCARD;// 使用此标志可以指定位块传输 (bitblt) 模型，并指定在调用 IDXGISwapChain1：:Present 后，DXGI 放弃后台缓冲区的内容。
	UINT flags = 0;

	ComQIPtr<IDXGIFactory5> factory5 = device->factory;
	if (factory5) {
		initData.num_backbuffers = max(data->num_backbuffers, 2);// 将 num_backbuffers 设置为 data->num_backbuffers 和 2 中的较大值，以确保至少有两个后备缓冲区。

		effect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
		flags |= DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT;// 设置标志 DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT，以便支持帧延迟等待对象。
	}
	...
	/* Ignore Alt+Enter */ // 调用 MakeWindowAssociation 方法，确保在窗口中按下 Alt+Enter 键时不会触发全屏模式。
